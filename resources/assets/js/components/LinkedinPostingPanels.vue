            ...
            <div class="card">
                ...
                <div class="content">
                    <template v-if="!accounts.length">
                        Nothing to see here.
                        <a :href="Laravel.routes['linkedin-account.index']">Add new account and its groups</a>.
                    </template>
            ...
            <div class="card">
                <template v-if="done">
                    <div class="header">
                        ...
                        <p class="category">
                            <a :href="`https://smp.roislope.com/linkedin/stats/${queue_id}`" target="_blank">
                                https://smp.roislope.com/linkedin/stats/{{ queue_id }}
                            </a>
                        </p>
                    </div>
                        ...
                        <form @submit.prevent="post">
                            <div class="form-group" :class="{ 'has-error': errors.client_id }">
                                ...
                                <template v-if="errors.client_id">
                                    <label v-for="errorMessage in errors.client_id" class="error">
                                        {{ errorMessage }}
                                    </label>
                                </template>
                            </div>
                            <div class="form-group" :class="{ 'has-error': errors.template_id }">
                                ...
                                <template v-if="errors.template_id">
                                    <label v-for="errorMessage in errors.template_id" class="error">
                                        {{ errorMessage }}
                                    </label>
                                </template>
                            </div>
...
<script>
    export default {
        ...
        computed: {
            selectedClientEmail() {
                return this.clients.filter(client => client.id === this.client_id)[0].email;
            }
        },
        methods: {
            post() {
                ...
                this.$http.post(Laravel.routes['linkedin.post'], data).then(response => {
                    this.done = true;
