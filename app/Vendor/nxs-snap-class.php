...
if (!class_exists("cl_nxsAutoPostToSN")) {
    ...
    {//## General Functions
        ...
        //## Constructor
        function __construct($networks, $optionsData)
        {
            $this->networks = $networks;
            $this->options = $this->setOptions($optionsData);
        }
        ...
        function setOption($nt, $num, $ntName, $value)
        {
            ...
            if (!is_array($opts[$nt])) $opts[$nt] = array();
            if (!is_array($opts[$nt][$num])) $opts[$nt][$num] = array();
            $opts[$nt][$num][$ntName] = $value;
            $this->options = $opts;
        }
        ...
        {
            $options = array();
            $dbOptions = maybe_unserialize($optionsData);
            if (!is_array($dbOptions)) die('No Settings found');
        ...
        {
            if (is_array($message)) $this->message = array_merge($this->message, $message); else $this->message['message'] = $message;
        }
        ...
        function autoPost()
        {
            ...
            $out = array();
            foreach ($networks as $nt) if (isset($nt['lcode'])) {
                $clName = 'nxs_class_SNAP_' . $nt['code'];
                if (!isset($options[$nt['lcode']])) continue;
                $ntClInst = new $clName();
                $out[$nt['lcode']] = $ntClInst->doPost($options[$nt['lcode']], $this->message);
            }
            return $out;
        }
        ...
        function showAllSettings()
        {
            ...
            $out = array();
            foreach ($networks as $nt) if (isset($nt['lcode'])) {
                $clName = 'nxs_snapClass' . $nt['code'];
                if (!isset($options[$nt['lcode']])) continue;
                $ntClInst = new $clName();
                $ntClInst->showGenNTSettings($options[$nt['lcode']]);
            }
        }
    }
}
