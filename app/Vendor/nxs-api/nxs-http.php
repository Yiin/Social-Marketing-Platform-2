...
if (!class_exists('nxs_Http')) {
    class nxs_Http
    {
        ...
        {
            global $nxs_version;
            $defaults = array('method' => 'GET', 'timeout' => '5', 'redirection' => 50, 'httpversion' => '1.1', 'follow-location' => false, 'user-agent' => 'Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0);',
            ...
            if (!is_array($r['headers'])) {
                $processedHeaders = nxs_Http::processHeaders($r['headers']);
        ...
        function sendReq($url, $type = 'GET', $args = array())
        {
            $defaults = array('method' => $type);
            $r = nxs_parse_args($args, $defaults);
        ...
        function curl_request($url, $args = array())
        {
            ...
            $r = nxs_parse_args($args, $defaults);
            if (isset($r['headers']['User-Agent'])) {
                $r['user-agent'] = $r['headers']['User-Agent']; /* unset($r['headers']['User-Agent']); */
            ...
            curl_setopt($handle, CURLOPT_FOLLOWLOCATION, $r['follow-location']);
            ...
            if (!empty($r['headers'])) {
                $headers = array();
                foreach ($r['headers'] as $name => $value) $headers[] = "{$name}: $value"; // prr($headers, "-WWX-".$r['method']." - ".$url);
            ...
            $response['message'] = nxs_getHttpStatusDesc($response['code']);
            curl_close($handle);
            if (!empty($theHeaders['headers']['location']) && 0 !== $r['_redirection']) { // _redirection: The requested number of redirections
                if ($r['redirection']-- > 0) {
                    return $this->request(nxs_HTTP::make_absolute_url($theHeaders['headers']['location'], $url), $r);
...
if (!class_exists('nxs_HTTP_Proxy')) {
    class nxs_HTTP_Proxy
    {
        //## NXS
        ...
        function is_enabled()
        {
            return isset($this->nxs_PROXY_HOST) && isset($this->nxs_PROXY_PORT);
        }

        function use_authentication()
        {
            return isset($this->nxs_PROXY_USERNAME) && isset($this->nxs_PROXY_PASSWORD);
        }

        function host()
        {
            if (isset($this->nxs_PROXY_HOST)) return $this->nxs_PROXY_HOST;
            return '';
        }

        function port()
        {
            if (isset($this->nxs_PROXY_PORT)) return $this->nxs_PROXY_PORT;
            return '';
        }

        function username()
        {
            if (isset($this->nxs_PROXY_USERNAME)) return $this->nxs_PROXY_USERNAME;
            return '';
        }

        function password()
        {
            if (isset($this->nxs_PROXY_PASSWORD)) return $this->nxs_PROXY_PASSWORD;
            return '';
        }
...
if (!class_exists('nxs_Http_Cookie')) {
    class nxs_Http_Cookie
    {
        var $name;
        var $value;
        var $expires;
...
if (!class_exists('nxs_Http_Encoding')) {
    class nxs_Http_Encoding
    {
...
if (!class_exists('nxs_Error')) {
    class nxs_Error
    {
        ...
        function get_errors()
        {
            if (empty($this->errors)) return array(); else return $this->errors;
        }
...
if (!function_exists("is_nxs_error")) {
    ...
    {
        if (is_object($thing) && (is_a($thing, 'nxs_Error') || is_a($thing, 'wp_Error'))) return true;
        return false;
    }
}
if (!function_exists("nxs_getHttpStatusDesc")) {
    function nxs_getHttpStatusDesc($code)
    {
...
if (!function_exists("nxs_parse_str")) {
    function nxs_parse_str($string, &$array)
    {
...
if (!function_exists("nxs_parse_args")) {
    ...
    {
        if (is_object($args)) $r = get_object_vars($args); elseif (is_array($args)) $r =& $args;
        else nxs_parse_str($args, $r);
        if (is_array($defaults)) return array_merge($defaults, $r);
        return $r;
    }
}

if (!function_exists("nxs_staticHttpObj")) {
    function nxs_staticHttpObj()
    {
        static $nxs_http;
        if (is_null($nxs_http)) $nxs_http = new nxs_Http();
        return $nxs_http;
    }
}
if (!function_exists("nxs_remote_request")) {
    ...
    {
        $nxs_http = nxs_staticHttpObj();
        return $nxs_http->request($url, $args);
    }
}
if (!function_exists("nxs_remote_get")) {
    ...
    {
        $nxs_http = nxs_staticHttpObj();
        return $nxs_http->sendReq($url, 'GET', $args);
    }
}
if (!function_exists("nxs_remote_post")) {
    ...
    {
        $nxs_http = nxs_staticHttpObj();
        return $nxs_http->sendReq($url, 'POST', $args);
    }
}
if (!function_exists("nxs_remote_head")) {
    ...
    {
        $nxs_http = nxs_staticHttpObj();
        return $nxs_http->sendReq($url, 'HEAD', $args);
    }
}
if (!class_exists('WP_Http_Cookie')) {
    class_alias('nxs_Http_Cookie', 'WP_Http_Cookie');
}
if (!function_exists("nxs_mkRemOptsArr")) {
    function nxs_mkRemOptsArr($hdrsArr, $ck = '', $flds = '', $p = '', $rdr = 0, $timt = 45, $sslverify = false)
    {
        ...
        if (!empty($flds)) $a['body'] = $flds;
        if (!empty($p)) $a['proxy'] = $p;
        if (!empty($ck)) $a['cookies'] = $ck;
        return $a;
    }
}
?>